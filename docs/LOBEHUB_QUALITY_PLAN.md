...
- [~] (3a) **保留（C待ち）**。実コード調査（lobehub/lobe-chat ＋ @lobehub/market-types v1.17.1）で確定: 「Skills」カウンターは登録メタデータの **静的 `PluginManifest.tools[]`** から populate される＝実行時 `listTools()` でも README 散文カタログでも flip しない（README に既存カタログがあるのに 0 なのが裏付け）。残る手は B: LobeHub が読む機械可読マニフェスト / C: claim → オーナー管理画面で期待フォーマットを実物確認。マニフェストの型は判明したが「リポジトリのどのファイルから読むか」は closed-source で不明なため、当て推量での skill.md 配置は避け、**claim 後の管理画面確認を待ってから B を判断**する。詳細スペックは `C:\Users\onetr\Downloads\lobehub-mcp-spec.txt`（子機調査）。
