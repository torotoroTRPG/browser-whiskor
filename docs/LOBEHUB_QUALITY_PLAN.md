...
**Quick wins（低コスト・体裁）**
- [ ] (3c) LobeHub バッジを README.md に追加 → 「所有者確認」flip。※バッジ markdown はユーザーが LobeHub からコピー
- [ ] (3e) README のインストール導線を整理（`whk setup` の容易さを前面に）

**Medium（実装・本物の機能 flip）**
- [ ] (3b) MCP `resources` capability を宣言・実装（例: 直近セッション/ステートグラフ/スクショを resource として公開）→「リソース」flip
- [ ] (3b) MCP `prompts` capability を宣言（例: 「このタブを調査して」等の定型プロンプト）→「注意喚起/prompts」flip
