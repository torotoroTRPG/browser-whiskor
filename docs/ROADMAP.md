...
### H. 実バグ修正
- **HTTP API への localhost CSRF（セキュリティ, 2026-06-10）**: `Access-Control-Allow-Origin` は応答の**読み取り**を拒否するだけで、`Content-Type: text/plain`+`mode:'no-cors'` の "simple request" は preflight なしで届き処理されていた。`appIsolation`(既定off)も無条件で素通り＝**ユーザーが訪れた任意のWebページが `/api/action`(click/navigate/type_text/execute_js等)でブラウザの別タブを操作可能**だった。`server/http-origin-guard.js` の `checkOrigin()` で、`Origin` ヘッダーが存在し許可オリジン外なら本文を読む前に403で拒否。`Origin` はページJSから偽装不可、未送出（同一オリジンGET/MCPプロキシ等の非ブラウザクライアント）は従来通り信頼。`allowedMcpOrigins:['*']`(明示的opt-out)とLAN公開(`host`がloopback以外)は対象外（`appIsolation`トークンが既存の対策パス）。`server/index.js`(L353周辺)。検証: `tests/unit/http-origin-guard.test.js`(7本、実モジュール)＋実サーバーでcurl実証(evil origin→403 / no-origin・same-origin・allowed origin→200)。✅ 🧪。
