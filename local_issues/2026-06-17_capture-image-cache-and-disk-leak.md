...
HTTP 画像返却の非対称（追記セクション）も**部分解決**: `/api/screenshot` が `format/quality/maxWidth/returnImage` を MCP 同様に反映するようになり、`GET /api/screenshots/:file` 配信ルート＋応答 `url` を新設（`returnImage:false` で base64 省略可）。**残り**＝「テキストファースト既定への反転」（base64 を既定で返さない）は dashboard.html と同梱 skill が `dataUrl` 依存のため破壊的＝製品判断として保留（2026-06-18: `httpInlineImage` config スイッチを追加し、非破壊で反転可能にした＝既定はまだ true）。webp フォーマット（理想機能メモ 項目10）は **2026-06-18 に要素系で実装**（capture_element_screenshot / get_element_thumbnail の `format:'webp'`）。フル screenshot/packed と AVIF は対象外（AVIF は却下）。
