    ...
    install(api) {
      ...
      if (navigator.sendBeacon) {
        ...
        navigator.sendBeacon = function (url, data) {
          api.emit('NETWORK_REQUEST', {
            source: 'page-js', reqId: `beacon-${Date.now()}`, url, method: 'BEACON',
            headers: {}, tokens: [],
      ...
      if (window.WebSocket) {
        ...
        const PatchedWS = function (url, protocols) {
          ...
          api.emit('NETWORK_REQUEST', { source: 'page-js', reqId, url: String(url), method: 'WS', kind: 'websocket', ts: Date.now() }, true);
          ...
          const flush = (force) => {
            ...
            lastFlush = now;
            api.emit('NETWORK_RESPONSE', { source: 'page-js', reqId, url: String(url), frames: summary(), ts: now }, true);
          };
          const note = (dir, data) => {
          ...
          ws.addEventListener('close', (ev) => {
            api.emit('NETWORK_RESPONSE', { source: 'page-js', reqId, url: String(url), status: (ev && ev.code) || 1000, frames: summary(), ts: Date.now() }, true);
          });
          ws.addEventListener('error', () => {
            api.emit('NETWORK_ERROR', { source: 'page-js', reqId, url: String(url), error: 'WebSocket error', ts: Date.now() }, true);
          });
          return ws;
        };
      ...
      if (window.EventSource) {
        ...
        const PatchedES = function (url, init) {
          ...
          api.emit('NETWORK_REQUEST', { source: 'page-js', reqId, url: String(url), method: 'SSE', kind: 'eventsource', ts: Date.now() }, true);
          ...
          es.addEventListener('message', (ev) => {
            ...
            const now = Date.now();
            if (now - lastFlush >= 1500) { lastFlush = now; api.emit('NETWORK_RESPONSE', { source: 'page-js', reqId, url: String(url), frames: summary(), ts: now }, true); }
          });
          es.addEventListener('error', () => {
            api.emit('NETWORK_RESPONSE', { source: 'page-js', reqId, url: String(url), status: es.readyState === 2 ? 'closed' : 'error', frames: summary(), ts: Date.now() }, true);
          });
          return es;
        };
