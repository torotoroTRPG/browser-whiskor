 ...
 *
 * Slice 1 (this file): known-value blacklist over collected payloads, applied at
 * the single core.js ingestion chokepoint. Pattern detection, screenshot masking
...
function createGuard(cfg) {
  ...
  const active  = enabled && secrets.length > 0;

  // Literal (non-regex) replace-all, so secret contents can't act as regex.
  function redactString(str) {
    if (!active || typeof str !== 'string' || !str) return str;
    let out = str;
    for (const s of secrets) {
      if (out.includes(s.value)) out = out.split(s.value).join(s.token);
    }
    return out;
  }
  ...
  return { enabled, active, count: secrets.length, redactString, redactDeep, redactMessage };
}

module.exports = { createGuard, makeToken, deriveHint, SECRETS_FILE };
