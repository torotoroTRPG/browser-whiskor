...
function getDefaults() {
  return {
    ...
    // allowedMcpOrigins fail-CLOSED default: localhost only, same as the shipped
    // config.json. '*' (allow all) must be an explicit user opt-in, never the
    // value a missing config falls back to.
    security:   { allowExecuteJs: false, allowActions: true, allowScreenshots: true, allowExplorer: true, executeJsTimeoutMs: 15000, actionTimeoutMs: 15000, allowedMcpOrigins: ['localhost', '127.0.0.1'] },
