...
server/index.js          ← エントリーポイント。HTTP:7892 + WS:7891 を立ち上げ
    ...
    ├── server/dev-intake.js     ← dev-exec artifact 静的検査 (bare/相対import拒否・SHA-256)
...
| `POST /api/dev/exec` | dev-exec 実行（`{tabId, code, mode?}`。**dev mode 非活性時は 404**＝不在の原則）。gate→intake→audit→注入→redact を経て verdict を返す。MCP `exec_module` と共有（proxy はここへ転送） |
