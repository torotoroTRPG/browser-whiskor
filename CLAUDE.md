...
| `POST /api/dev/exec` | dev-exec 実行（`{tabId, code\|path\|artifactId, mode?}`。**dev mode 非活性時は 404**＝不在の原則）。gate→intake→audit→注入→redact を経て verdict を返す。MCP `exec_module` と共有（proxy はここへ転送） |
