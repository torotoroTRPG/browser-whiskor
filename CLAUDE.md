...
server/index.js          ← エントリーポイント。HTTP:7892 + WS:7891 を立ち上げ
    ...
    │               ├── write.js       ← 18 writeツール (type_secret / unhover 含む。click/type等は observe オプションで操作後の状態ハッシュ安定を観測可能。click/drag は while:{keys} 宣言的保持、drag は plan/observed 二層レポート)
    ...
    ├── server/action-executor.js← アクション実行ルーター (type_secret の ref→値解決・前提変化feedのアクション窓マーキングもここ)
