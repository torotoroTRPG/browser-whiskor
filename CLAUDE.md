...
whk config preset dev             # 開発者向けキー一式を列挙表示しつつ config.local.json へ書込（既存値優先、--force で上書き、--dry-run あり）。privacy.* と dev mode 活性化には触れない。実体は server/config-presets.js
...
whk dev off                       # dev mode 失効
whk dev status                    # 現在の dev mode 状態
whk dev exec <file.js> --tab <id> [--harness]  # 成果物(自己完結ESモジュール)を実タブで実行し verdict を返す
...
`package.json` の `version` が**唯一の真実**。`extension/manifest.json` と `firefox-mv2/manifest.json` と `docs/version.json`（GitHub Pages が配信し、サーバーの `updateCheck` が参照する公開バージョン）はこれに追従し、`npm run check-version`（実体は `scripts/_check-version.js`）が3ファイルの一致を検証する。CI (`ci.yml` の `verify-sync`) と `validate.ps1` の両方がこのチェックを走らせるため、ズレたまま push すると落ちる。
...
server/index.js          ← エントリーポイント。HTTP:7892 + WS:7891 を立ち上げ
    ...
    ├── server/mcp-server.js     ← MCP層 (72ツール)
    ...
    │               ├── write.js       ← 18 writeツール (type_secret / unhover 含む。click/type等は observe オプションで操作後の状態ハッシュ安定を観測可能。diff オプションで結果に _diff＝要素単位の差分を添付。click/drag は while:{keys} 宣言的保持、drag は plan/observed 二層レポート)
    ...
    │               ├── intelligence.js← 7 intelligenceツール (analyze_click / get_canvas_map / get_state_map_visual 含む)
    │               ├── ocr.js         ← ocr_region (ピクセルからのOCR読取。canvas/WebGL・アイコンのみ要素向け)
    │               ├── source.js      ← get_source_context + capture_sources (DevToolsソースキャプチャ)
    │               ├── replay.js      ← replay_session
    │               └── dev.js         ← exec_module / dev_status (dev-exec。dev mode 活性時のみ可視)
    ├── server/dev-gate.js       ← dev mode 状態機械 (明示活性化・TTL失効・不在原則。config値ではない)
    ├── server/dev-intake.js     ← dev-exec artifact 静的検査 (bare/相対import拒否・SHA-256・fileRoots閉じ込め)
    ├── server/dev-artifacts.js  ← push intake の成果物LRU (メモリのみ・ディスク非永続)
    ├── server/dev-verdict.js    ← 判定エンジン (baseline/observed→5値verdict＋evidence。verdicts.jsonl 永続化)
    ├── server/dev-audit.js      ← dev-exec 監査 (audit-before-ack。cache/sessions/{tabId}/dev/audit.jsonl)
    ├── server/tool-manager.js   ← 動的プロファイルロード/アンロード (ALWAYS_VISIBLE_TOOLS。dev profileは不在原則で別扱い)
    ├── server/cache-writer.js   ← ディスク永続化 (cache/sessions/{tabId}/raw/...)
    ├── server/cache-integrity.js← 起動時キャッシュ検証・自動修復
    ├── server/action-executor.js← アクション実行ルーター (type_secret の ref→値解決・前提変化feedのアクション窓マーキング・action.diff の diff runner 呼び出しもここ)
    ├── server/action-diff.js    ← アクション連動diff (baseline→scoped collect→要素単位突合。agentControl.actionDiff)
    ├── server/change-feed.js    ← 前提変化feed (外部変化のタブ別リングバッファ。_sinceYourLastLook piggyback)
    ...
    ├── server/source-store.js       ← ソースコンテンツ永続化 (hash/dedup/binary)
    ...
    ├── server/state-navigator.js   ← BFS経路探索 + アクションリプレイ + 推測逆エッジ(go_back/Escape/閉じるラベル候補をfindPath時にlazy生成、検証成功でbasis付き永続化=earned、失敗はプロセス内blacklist+再プラン) + fuzzyターゲット解決(mode=strict/auto/fuzzy、matched:'fuzzy'+similarityで正直に報告)。URLフォールバックは fallback:'url'+note 付き
...
| **advanced-actions** (+13) | 自動 | drag, hover, unhover, select_option, check_box, mouse_scroll, right_click, press_key, type_secret, go_back, go_forward, reload_page, scroll_page | "drag", "hover", "select" | 5ターン |
| **tabs** (+4) | 自動 | list_tabs, switch_tab, open_tab, close_tab | "switch tab", "new tab", "popup", "redirect" | 6ターン |
| **intelligence** (+8) | 自動 | explain_element, why_did_this_change, get_source_file, detect_site_updates, get_source_context, capture_sources, ocr_region, get_canvas_map | "explain", "why", "source", "cause", "ocr", "canvas" | 5ターン |
...
| **dev** (+2) | **dev mode 活性時のみ** | exec_module, dev_status | — (自動トリガーなし) | mode 失効で消滅 |

**dev プロファイルは「不在の原則」で別扱い**（`docs/vision/whiskor-for-dev/dev-exec.md`）。dev mode が非活性の間はツールが `tools/list` に**現れず**（存在自体を告知しない）、`/api/dev/exec` は 404。activate/失効で `tools/list_changed` が発火する。activate は operator 専権（`whk dev on`／MCP からは不可能＝I-2）、常時可視（拡張バッジ）、TTL/再起動で失効（＝config値ではない実行時状態）。`staticTools` でも dev だけは常時公開の対象外。
...
- `!<command>` — ユーザーのローカルシェル（Windows: pwsh→powershell→cmd、POSIX: $SHELL）でコマンド実行。stdout/stderrをscrollbackに表示。非対話・30sタイムアウト。HTTP/MCPには**一切出さない**。classic シェルにも同等あり
...
開発者向けの一括有効化は **`whk config preset dev`**（`server/config-presets.js`）：既知の能力系キー（allowExecuteJs / allowAgentConfig / dev.exec.enabled / highFidelity=fallback / captureAllWorlds / actionDiff.auto / packedSom prefetch×2）を**1件ずつ列挙表示しながら** config.local.json に書き込む。既存値は保持（`--force`で上書き）、**将来追加分は再実行で明示的に取り込む**（単一マスターフラグ＝将来キーへの白紙委任は採らない設計）。privacy.*（保護系）とネットワーク露出には触れず、dev **mode** の活性化も従来どおり `whk dev on` が必要。雛形は `config.local.json.example`（preset とドリフトしないよう契約テストで固定）。
...
- `dev.exec.enabled`: デフォルト `false`。dev-exec（ビルド済み成果物を実ページで**ライブ実行**し判定ループを回す）のポリシー総開閉。`false` なら `whk dev on` 自体が拒否される。有効化は個人値ゆえ `config.local.json` へ（CIガード `_check-config-defaults.js` が `dev.exec.enabled=false`／`dev.exec.fileRoots=[]` の公開既定を検証）。dev **mode**（活性な実行時状態）は config には持たない＝operator が `whk dev on` で明示活性化・拡張バッジで常時可視・TTL（`dev.mode.defaultTtlMs` 既定4h／上限 `maxTtlMs` 24h）と再起動で失効。`execute_js`（式1つ評価・`allowExecuteJs`ゲート）とは別ツール・別ゲート（`exec_module`＝自己完結ESモジュール1本を注入・dev modeゲート）。agent からは `dev.*` 全キー read-only。実体は `dev-gate.js`（mode状態機械）＋`dev-intake.js`（静的検査・hash・fileRoots閉じ込め）＋`dev-artifacts.js`（push intake LRU）＋`dev-verdict.js`（判定エンジン）＋`dev-audit.js`（audit-before-ack）＋executor の `execute_module` action（blob注入・MAIN world・baseline/settle/observed）。設計は `docs/vision/whiskor-for-dev/dev-exec.md`（E1実行プリミティブ＋ゲート＋監査／E2 file・push intake／E3 判定ループ verdict engine 実装済。cdp backend・watchループ・慣習レシピは後続）
...
- `agentControl.console.captureAllWorlds`: デフォルト `false`。計測中タブの**全実行コンテキスト**の console/例外を CDP (`chrome.debugger`) で捕捉する。他拡張の content script は自分専用の ISOLATED world でログを吐き、MAIN world の console-logger アナライザーからは構造的に見えない——その盲域（他拡張の注入検証など）を埋める。エントリは `{world, via:'cdp'}` タグ付きで既存の `CONSOLE_LOG` パイプラインに合流（MAIN world は従来どおりアナライザー担当＝二重報告なし）。`get_console_logs` の `world` 引数でフィルタ可（`"main"`=ページのみ）。**Chrome専用**（Firefox に debugger API なし）。デバッガを保持し続けるためバナーが常時出る。有効化後はタブの次のデータ送信時にアタッチし、入力 highFidelity とアタッチを共有（tap 保持中は idle-detach が譲る）。公開既定 `false` は `_check-config-defaults.js` がガード。実体は `extension/background/sw.js` の `cdpConsoleTap`
- `agentControl.changeFeed`: デフォルト `enabled:true`。**前提変化feed** — アクション実行窓（実行中＋解決後 `actionTrailMs` 既定1.5s）の**外**で起きたページ変化（scroll＝coalesceして最終位置1行 / modal＝role=dialog等の出現消滅 / 遷移）をタブ別リングバッファ（`maxEntries` 既定50）に貯め、次のMCPツール応答へ `_sinceYourLastLook` として同乗させる（読取＝既読クリア、タブ閉鎖で破棄。中央添付は `mcp/registry.js`）。click/type_text/drag の `abortOnPremiseChange:true` で「外部変化が溜まっていたら実行せず中断」の前提ゲートになる。HTTPは `GET /api/changes/:tabId`。設計は `docs/ideas/PREMISE_CHANGE_FEED.md`
- `agentControl.actionDiff`: デフォルト `auto:false`。**アクション連動diff** — アクションの `diff:true` 指定（`auto:true` なら全ページアクションで既定ON、`diff:false` で個別OFF）で、結果に `_diff`＝「エージェントが最後に見た収集データからの要素単位の変化」を添付する。interactive の出現/消失/移動（layout-map と同じ kind+label+座標語彙＝そのまま click ターゲットに使える）、テキストの追加/削除/変化（同位置の削除+追加は1件の changed に対合）、スクロール。基準はキャッシュの前回値、アクション後に `text-coords`+`ui-catalog` の scoped collect（`settleDelayMs` 既定300ms 待ち→`collectTimeoutMs` 既定3s まで freshness poll）で更新後を取得して突合。各リスト `maxEntries`（既定15）でキャップ、counts は常に全数。操作→再読取の往復を1回に畳む。実体は `server/action-diff.js`（純関数diff＋runner）＋`action-executor.js` の注入呼び出し（worker側実行なので MCP stdio/HTTP/proxy で同一動作）。注意: runner の getConfig は**フル設定 `_cfg`** を読む（`core.globalConfig` は SW向けサブセットで actionDiff を含まない——配線ピンテストで固定済み）
...
- `updateCheck`: デフォルト `enabled:true`。通常（非MCP）起動時に `docs/version.json`（GitHub Pages＝`main:/docs` 配信、`npm version` で自動同期。実体は `scripts/_check-version.js` が manifest と一緒に書く）を fetch し、`package.json` 版と semver 比較する。**バージョン確認のみ＝コードのDL/実行はしない**。新版はログ＋`GET /health` の `update`＋ダッシュボードバナー＋デスクトップ通知（`osToast:true`、同梱 `scripts/_notify.js`＝クロスプラットフォーム、Windows は pwsh+BurntToast 優先、無ければ印字にフォールバック）で知らせる。`notifyCommand`（非空でカスタム上書き、`{current}/{latest}/{tag}/{url}` 置換）。`autoSetup:true`（opt-in）は検出時に `whk setup` を走らせ**ローカル既存**拡張を管理ディレクトリへ反映＋リロード（新版DLはしない＝自分でコード更新後にのみ有効）。`selfUpdate` は将来のDL＆置換アップデータ用の予約 seam（現状 inert、設計は `docs/ideas/SELF_UPDATE.md`）。実体は `server/update-checker.js`（純ロジックで unit テスト済）＋ `index.js` 起動フック＋ `core.js` の `/health`。agent からは read-only（`permissions.agentReadOnlyPaths`）
- `network.redactAuthHeaders`: デフォルト `true`（**常時ONの基盤保護**、secretGuard とは別）。認証系HTTPヘッダの**値**（Authorization/Cookie/Set-Cookie/X-Api-Key/X-Csrf-Token/Proxy-Authorization 等）を**収集時点**（emit 前、ページ側 network.js）で `[redacted len=N]` 化する。キャッシュ・ダッシュボード・`/export`・agent 到達より前に処理＝opt-in の secretGuard に依存しない。ヘッダ名ベース（全サイト有効・同期不要）。API デバッグで生ヘッダが要るときのみ `false`。本文/フォーム値は対象外（それは secretGuard）。network.js は shared 外＝両拡張を直接編集。公開既定 `true` は `_check-config-defaults.js` がガード。実体は両 `injected/analyzers/network.js` の `redactHeaders`
...
| `GET /health` | 接続確認（`update` に起動時バージョンチェック結果＝`{current,latest,updateAvailable,tag,url}` を含む） |
| `GET /` | ダッシュボード |
...
| `GET /api/dev/status` | dev-exec mode の状態（`{active, expiresAt, remainingMs, roots(数のみ), policyEnabled}`）。`whk dev status` が使用 |
| `POST /api/dev/on` | dev mode 活性化（operator専権。`{ttlMs?, project?}`。常に200＝ok:false理由も本文で返す）。`whk dev on` が使用 |
| `POST /api/dev/off` | dev mode 失効。`whk dev off` が使用 |
| `POST /api/dev/exec` | dev-exec 実行（`{tabId, code\|path\|artifactId, mode?}`。**dev mode 非活性時は 404**＝不在の原則）。gate→intake→audit→注入→settle→redact を経て **verdict（5値＋evidence）** を返す＋`verdicts.jsonl` へ永続化。MCP `exec_module` と共有（proxy はここへ転送） |
| `POST /api/dev/artifact` | push intake（`{name, code}` or `{name, zipBase64}`＝単一.jsのzip）→ `{artifactId, hash}`。以後 `exec_module{artifactId}` で参照実行。**非活性時404**。本体はメモリLRU（`dev.exec.artifactCacheMax`）でディスク非永続（I-4） |
...
| `GET /api/sessions/:tabId/canvas-map` | canvas の**中身**を framework state から描画（store/props の数値 x/y を自動発見 → content bbox クロップ → 密度で grid/list 自動選択。座標は store 単位）。`?path=&hints=<JSON>&width=&form=auto\|grid\|list&legend=&canvasIndex=&maxObjects=`。実体は `server/canvas-map.js`＝MCP `get_canvas_map` と共有 |
...
| `POST /api/source/capture` | DevTools経由ソースキャプチャ（`{tabId, includeBinary?, includeNetwork?, reload?, timeoutMs?}`。パネルが開いている必要あり） |
| `POST /api/source/upload` | プロジェクトソースのアップロード (ファイル群 or base64 zip) |
| `POST /api/source/context` | アップロード済ソースのスライス取得 |
| `GET /api/sources/:tabId` | キャプチャ済みソースファイル一覧 |
| `GET /api/sources/:tabId/zip` | キャプチャ済みソースをフォルダ構造付きZIPでダウンロード |
