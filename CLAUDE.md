...
server/index.js          ← エントリーポイント。HTTP:7892 + WS:7891 を立ち上げ
    ...
    │       └── mcp/tools/
    │               ├── read-basic.js / read-data.js / read-state.js (+read.js/read-helpers.js) ← 22 readツール
...
常時公開は16ツールの `core` プロファイルのみ。他のプロファイルはキーワード自動検出またはAIの明示的なロードで動的に有効化・無効化される。`search_tools` / `load_profile` / `unload_profile` / `profile_status` / `analyze_click` の5つは常時公開される「メタツール」（`server/tool-manager.js` の `ALWAYS_VISIBLE_TOOLS`）。
...
| **intelligence** (+5) | 自動 | explain_element, why_did_this_change, get_source_file, detect_site_updates, get_source_context | "explain", "why", "source", "cause" | 5ターン |
