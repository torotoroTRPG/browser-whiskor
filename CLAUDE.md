...
server/index.js          ← エントリーポイント。HTTP:7892 + WS:7891 を立ち上げ
    ...
    ├── server/state-navigator.js   ← BFS経路探索 + アクションリプレイ + 推測逆エッジ(go_back候補をfindPath時にlazy生成、検証成功でbasis付き永続化=earned、失敗はプロセス内blacklist+再プラン。URLフォールバックは fallback:'url'+note で正直に報告)
